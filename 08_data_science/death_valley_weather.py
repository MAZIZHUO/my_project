...
file_path = current_dir / "weather_data" / "death_valley_2021_simple.csv"
...
for row in reader:  # 遍历剩余的行
    try:  # 当数据出现缺失值时，会抛出异常，导致程序崩溃。为了避免这种情况，我们可以使用 try-except 块来捕获异常，并在发生异常时打印错误信息，而不是让程序崩溃。
        current_date = datetime.strptime(row[2], "%Y-%m-%d")  # 获取当前行的日期
        high = int(row[3])  # 获取当前行的最高温度
        low = int(row[4])  # 获取当前行的最低温度
    except ValueError as e:  # 如果在转换过程中发生 ValueError 异常，说明数据有问题
...
# 3. 使用 Matplotlib 绘制最高温度和最低温度的图表
